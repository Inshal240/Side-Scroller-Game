...
JointPlatform::JointPlatform( sf::Texture& texture, float initX, float initY, bool isCircular)
{
	_drawJoint = false;
	...
	_bodyShape.m_radius = MathHelper::ToUnit( texture.getSize().y / 2.f );
	...
	_jointBodyShape.m_radius = MathHelper::ToUnit( 5.f );
...
JointPlatform::JointPlatform(sf::Texture& texture, sf::Texture& jointTexture, float initX, float initY, float rotation)
{
	...
	_bodyShape.m_radius = texture.getSize().y ;
