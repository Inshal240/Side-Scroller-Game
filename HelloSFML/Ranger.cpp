...
void Ranger::LoadContent()
{
	...
	_animationBank[Dead].addFrame(IntRect(0, -5, 47 - 0, 67));
...
void Ranger::handleEvent(Event gameEvent, Player& player)
{
	...
	if (playerFound && !_inAir)
	{
		...
		// Judge whether to attack or not with a chance of 1/20
...
void Ranger::Update(Event gameEvent, Player& player, Time dt, Time frameTime)
{
	...
	if (_state == Idle || _state == Attack)
	{
		if (playerOnRight == 1 && _animatedSprite.getScale().x == -1)
		{
			_animatedSprite.scale(-1.f, 1.f);
		}
		else if (playerOnRight == -1 && _animatedSprite.getScale().x == 1)
		{
			_animatedSprite.scale(-1.f, 1.f);
		}
	}
	...
	if (_knifeThrown)
	{
		_pKnife = new Projectile(_knifeText,
			_animatedSprite.getPosition().x + 10 * playerOnRight,
	...
	{
		_pBody->GetWorld()->DestroyBody(_pKnife->GetPhysicsBody());
		_knifeExists = false;
	}
	...
	if (_state == Dead && !_animatedSprite.isPlaying())
	{
		...
		if (_knifeExists)
		{
			_pKnife->GetPhysicsBody()->GetWorld()->DestroyBody(_pKnife->GetPhysicsBody());
			_knifeExists = false;
		}
	}
