...
{
	bool attacking = _state == Attack1 || _state == Attack2 || _state == Attack3;
	// When player is grounded
	if (!_inAir)
	{
		// Running and jumping
		if (Keyboard::isKeyPressed(Keyboard::LShift) && !attacking)
		{
		...
		{
			// Jumping
			if (Keyboard::isKeyPressed(Keyboard::Up) && !attacking)
			{
				if (Keyboard::isKeyPressed(Keyboard::Right))
				{
...
void Player::LoadContent()
{
	...
	// Dead Animation
	_animationBank[Dead].setSpriteSheet(_textureBank[Hurt]);
	_animationBank[Dead].addFrame(IntRect(44, 0, 85 - 44, 47));
...
int Player::GetCurrentFrame() const { return _animatedSprite.getCurrentFrame(); }

void Player::Respawn()
{
	MoveTo(_spawnPoint.x, _spawnPoint.y);
	_alive = true;
	_health += 200;
}
...
{
	_spawnPoint = b2Vec2(x, y);
}

b2Vec2 Player::GetSpawnPoint() const
{
	return _spawnPoint;
}
...
{
	return _animatedSprite.getScale().x;
}